...
from utils.auth import token_required, cliente_required, atendente_required, admin_required # Decoradores mais específicos
...
@cliente_required # Apenas clientes criam agendamentos para si
def create_agendamento_cliente():
    data = request.json
    id_cliente_logado = request.current_user['id_usuario']

    # Forçar id_cliente a ser o do usuário logado
    data['id_cliente'] = id_cliente_logado
    
    # Remover 'id_servico' da validação se não for mais usado
    # A validação atual espera 'id_servico'
    # Se você removeu 'id_servico' do payload, precisa ajustar validate_agendamento_data
    # Por ora, vamos assumir que validate_agendamento_data foi ajustada para não exigir id_servico
    
    ag_errors = validate_agendamento_data(data) # Ajustar para não exigir id_servico
    if ag_errors:
    ...
    try:
        ...
        with connection.cursor() as cursor:
            ...
            # Verificar disponibilidade do atendente (simplificado, idealmente uma função mais robusta)
            # Esta query deve ser mais robusta, verificando sobreposição com data_hora_inicio e duracao_minutos
            # A função fn_verificar_disponibilidade_atendente do SQL já faz isso
            cursor.execute(
            ...
            # Notificar o atendente (Opcional, mas bom)
            ...
            # Retornar o agendamento criado para o frontend
...
def recusar_agendamento_pelo_atendente(agendamento_id):
    ...
    try:
        ...
        with connection.cursor() as cursor:
            ...
            # Atualiza o status e adiciona o motivo nas observações do atendente (ou criar campo específico)
            cursor.execute("""
...
def cancelar_agendamento_pelo_cliente(agendamento_id):
    id_cliente_logado = request.current_user['id_usuario']
    # data = request.json (motivo pode ser opcional para cliente)
    # motivo_cancelamento = data.get('motivo', 'Cancelado pelo cliente.')
    connection = None
    try:
        connection = get_connection()
        with connection.cursor() as cursor:
            ...
            # Regra de antecedência (ex: 24h) - opcional
            # if ag['data_hora_inicio'] - datetime.now() < timedelta(hours=24) and ag['status_agendamento'] == 'CONFIRMADO':
            #     return jsonify({'message': 'Cancelamento muito próximo. Entre em contato com o atendente.'}), 400
...
def marcar_agendamento_realizado(agendamento_id):
    ...
    try:
        ...
        with connection.cursor() as cursor:
            ...
            # Notificar cliente para avaliar (Opcional)
            # ...
...
# Rota para buscar agendamentos de um cliente específico (usada pelo frontend meus-agendamentos)
# Esta rota pode ser combinada com a rota principal /api/agendamentos se o token_required já filtrar pelo usuário
@agendamentos_bp.route('', methods=['GET']) # Se token_required estiver ativo, ele já filtra pelo usuário
@token_required
def get_meus_agendamentos_filtrados(): # Nome da função diferente para evitar conflito com a pública de cima
    user_id = request.current_user['id_usuario']
    user_type = request.current_user['tipo_usuario']
    
    # Parâmetros de filtro (opcional, o frontend pode pedir todos e filtrar lá)
    ...
    if user_type == 'CLIENTE':
        ...
        params.append(user_id)
    elif user_type == 'ADMIN': # Admin pode ver todos, mas esta rota é mais para /meus
        # Se admin precisa de uma visão geral, usar um endpoint /admin/agendamentos
        base_query += " 1=1 " # Se admin acessar /meus, mostra todos, mas é estranho. Melhor rota separada.
    else:
    ...
    # Separar em futuros e passados se o frontend não fizer
    agora = datetime.now()
...
@agendamentos_bp.route('/avaliacoes', methods=['POST']) # Movido de /<id>/avaliar para rota própria
@cliente_required # Apenas clientes podem criar avaliações
def criar_nova_avaliacao():
    data = request.json
    id_cliente_logado = request.current_user['id_usuario']
    
    # Adicionar id_avaliador ao payload
    data['id_avaliador'] = id_cliente_logado
    # id_avaliado (atendente) será pego do agendamento
    ...
    try:
        connection = get_connection()
        with connection.cursor() as cursor:
            # Verificar se o agendamento existe, pertence ao cliente, e está REALIZADO
            cursor.execute("""
...
def admin_cancelar_agendamento(agendamento_id):
    ...
    try:
        ...
        with connection.cursor() as cursor:
            ...
            if ag['status_agendamento'] in ['REALIZADO', 'CANCELADO_CLIENTE', 'CANCELADO_ATENDENTE', 'CANCELADO_ADMIN']: # Adicionar CANCELADO_ADMIN
            ...
            # Usar um status específico ou um campo para registrar quem cancelou
            novo_status = 'CANCELADO_ADMIN' # Ou um mais genérico e logar a ação
            cursor.execute("UPDATE agendamento SET status_agendamento = %s WHERE id_agendamento = %s", (novo_status, agendamento_id))

            # Log da ação do admin (você pode ter uma tabela de log de ações administrativas)
            # Ex: INSERT INTO admin_log (id_admin, acao, id_alvo, detalhes) VALUES (...)

            # Notificar cliente e atendente
        ...
        if connection: connection.close()
# Rota /api/agendamentos/horarios-disponiveis já foi movida para /api/atendentes/<id>/disponibilidade
# A rota de dashboard foi removida pois não estava sendo usada pelo frontend que projetamos. Pode ser adicionada depois se necessário.