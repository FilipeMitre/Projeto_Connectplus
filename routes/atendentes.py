# routes/atendentes.py
from flask import Blueprint, request, jsonify
from utils.db import execute_query, get_connection # Adicionado get_connection
from utils.auth import token_required, admin_required, atendente_required # Adicionado atendente_required
from utils.validators import validate_atendente_detalhes_data # Adicionado validador
from datetime import datetime, timedelta  # Adicionando importação do datetime

atendentes_bp = Blueprint('atendentes', __name__)

@atendentes_bp.route('', methods=['GET']) # Usado por agendamento.html e admin-panel.html
# @token_required # Geralmente a lista de atendentes é pública, mas se precisar de token, adicionar.
def get_atendentes_publico():
    situacao_filtro = request.args.get('situacao', 'ATIVO') # Por padrão, apenas ATIVOS para busca pública
    area_filtro = request.args.get('area_atuacao') # Corrigido para area_atuacao conforme frontend
    busca_filtro = request.args.get('busca')

    # Usar a view vw_atendentes_aprovados que já tem média de avaliação e outros detalhes
    # Ou construir uma query mais complexa aqui
    query = """
        SELECT 
    ...
    if situacao_filtro and situacao_filtro != 'TODOS': # 'TODOS' pode vir do admin panel
    ...
    query += " ORDER BY media_avaliacoes DESC, u.nome_completo ASC" # Exemplo de ordenação
    
    # Adicionar limite se vier do frontend
...
@token_required # Qualquer usuário logado pode ver perfil público, ou apenas atendente/admin
def get_perfil_atendente(atendente_id):
    # Esta rota pode ser para o perfil público (se chamada sem ser o próprio atendente/admin)
    # ou para o próprio atendente editar (se for o request.current_user['id_usuario'])
    # Reutiliza a lógica de /api/usuarios/me, mas focado em atendente
    ...
    try:
        with connection.cursor() as cursor:
            ...
            # Telefones e endereços (se for para o próprio atendente editar)
            # Para perfil público, pode omitir ou mostrar apenas informações de contato comercial
            telefones_info = []
            enderecos_info = []
            ...
            return jsonify({
                ...
                'telefones': telefones_info, # Para edição pelo próprio atendente
                'enderecos': enderecos_info   # Para edição pelo próprio atendente
            }), 200
    finally:
...
@atendente_required # Apenas o próprio atendente pode editar seu perfil (ou admin por outra rota)
def update_perfil_atendente(atendente_id):
    ...
    # Reutiliza a lógica de /api/usuarios/<id>/perfil (PUT) mas focada no atendente
    # O payload do frontend é { usuario: {...}, detalhes: {...} }
    data = request.json
    ...
    try:
        ...
        connection.begin()
        with connection.cursor() as cursor:
            # 1. Atualizar tabela `usuario` (campos permitidos)
            user_errors = validate_user_data(usuario_data, is_update=True) # Validar dados do usuário
            if user_errors:
                connection.rollback()
            ...
            tel_principal_num = usuario_data.get('telefone_principal') # Ou como vier do frontend
            if tel_principal_num:
                 # Lógica para atualizar/inserir telefone principal (semelhante a /usuarios/<id>/perfil)
                ...
                cursor.execute("UPDATE telefone SET is_principal = FALSE WHERE id_usuario = %s", (atendente_id,)) # Limpa principal antigo
            ...
            # 2. Atualizar tabela `atendente_detalhes`
            ...
            details_update_payload = {k: v for k, v in detalhes_data.items() if k in allowed_details_fields} # Permite None para limpar
            if details_update_payload:
                ...
                else: # Inserir se não existir (raro, mas para robustez)
...
@admin_required
def aprovar_atendente_admin(id_atendente): # Renomeado para evitar conflito de nome
    data = request.json
    ...
    try:
        connection = get_connection()
        with connection.cursor() as cursor:
            # Verifica se o usuário é um atendente e está PENDENTE_APROVACAO
            ...
            # Notificação (opcional, mas bom para o frontend)
            cursor.execute("""
...
@atendentes_bp.route('/<int:id_atendente>/bloquear', methods=['POST']) # Usado para reprovar também
@admin_required
def bloquear_atendente_admin(id_atendente): # Renomeado
    data = request.json
    ...
    try:
        ...
        with connection.cursor() as cursor:
            ...
            cursor.execute("""
                INSERT INTO notificacao (id_usuario_destino, titulo, mensagem, tipo_notificacao)
                VALUES (%s, %s, %s, %s)
            """, (id_atendente, 'Aviso sobre sua Conta', f'{tipo_notificacao_msg} Motivo: {motivo}', 'ATENDENTE_CADASTRO_REPROVADO')) # Usar tipo REPROVADO para ambos os casos por simplicidade
...
@token_required # Atendente pode ver suas próprias, admin pode ver de qualquer um.
def get_avaliacoes_do_atendente(id_atendente):
    ...
    # Se não for admin e não for o próprio atendente, nega (embora @atendente_required já possa ter feito isso)
...
def get_agendamentos_atendente(id_atendente):
    # Verificar se o usuário logado é o próprio atendente ou um admin
    if request.current_user['tipo_usuario'] == 'ATENDENTE' and request.current_user['id_usuario'] != id_atendente:
...
# @token_required # Geralmente público para quem está agendando
def get_disponibilidade_atendente_api(id_atendente): # Renomeado para evitar conflito
    data_str = request.args.get('data') # YYYY-MM-DD
    # duracao_solicitada = request.args.get('duracao', type=int) # Frontend pode enviar
    ...
    try:
        with connection.cursor() as cursor:
            # Obter duração padrão do atendente
            ...
            # if duracao_solicitada and duracao_solicitada > 0: # Se o frontend puder solicitar duração específica
            #     duracao_intervalo = timedelta(minutes=duracao_solicitada)
            # else:
            duracao_intervalo = timedelta(minutes=duracao_padrao or 60)


            # Buscar agendamentos CONFIRMADOS ou SOLICITADOS para o dia
            cursor.execute("""
            ...
            # Definir horários de trabalho (exemplo: 8h-12h, 14h-18h)
            # Isso poderia vir do BD por atendente no futuro
            horarios_trabalho = [
            ...
            for inicio_turno, fim_turno in horarios_trabalho:
                ...
                while slot_atual + duracao_intervalo <= fim_turno_dt:
                    if slot_atual < datetime.now() and data_obj == datetime.now().date(): # Não agendar no passado no mesmo dia
                        slot_atual += duracao_intervalo # Pular este slot
                        continue
                    ...
                    slot_atual += duracao_intervalo # Próximo slot baseado na duração padrão
