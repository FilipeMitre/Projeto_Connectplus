...
from utils.db import execute_query, get_connection # Adicionar get_connection
from utils.auth import token_required, admin_required, hash_password, check_password # Adicionar check_password
from utils.validators import validate_user_data, validate_telefone_data, validate_endereco_data # Adicionar validadores
...
def get_meu_perfil():
    ...
    try:
        with connection.cursor() as cursor:
            ...
            if not telefone_info: # Tenta pegar qualquer um se não houver principal
            ...
            if not endereco_info: # Tenta pegar qualquer um se não houver principal
                cursor.execute("""
                    SELECT logradouro, numero, complemento, bairro, cidade, estado, cep, tipo_endereco 
            ...
            # Se for atendente, buscar detalhes profissionais
...
@token_required # Usuário pode atualizar seu próprio perfil, ou admin pode atualizar qualquer um
def update_meu_perfil(user_id):
    # Segurança: Garante que o usuário só pode editar seu próprio perfil, a menos que seja admin
    ...
    # O payload do frontend para perfil de CLIENTE é mais simples
    # { nome_completo, nome_social, data_nascimento, identidade_genero, orientacao_sexual, pronomes, telefone_principal, cep_principal }
    # O payload para perfil de ATENDENTE é { usuario: {...}, detalhes: {...} }

    try:
        connection = get_connection()
        connection.begin()
        with connection.cursor() as cursor:
            # Dados do usuário (tabela usuario)
            usuario_update_data = data.get('usuario', data) # Se 'usuario' não existir no payload, assume que 'data' é para a tabela usuario
            ...
            # Atualizar telefone principal (se fornecido)
            # O frontend envia 'telefone_principal' (cliente) ou 'telefone_numero' (atendente no obj usuario)
            ...
            if tel_principal_num:
                ...
                # Verifica se já existe, senão insere
                ...
                else: # Tenta atualizar qualquer telefone ou insere novo como principal
            ...
            # Atualizar CEP principal (simplificado, apenas CEP)
            cep_principal_num = usuario_update_data.get('cep_principal') # Do perfil cliente
            # Se o perfil do atendente enviar dados de endereço, precisaria tratar aqui.
            # Para uma atualização completa de endereço, seria melhor um endpoint dedicado ou um payload mais rico.
            if cep_principal_num:
                # Apenas atualiza o CEP do endereço principal, se existir
                cursor.execute("UPDATE endereco SET cep = %s WHERE id_usuario = %s AND is_principal = TRUE", (cep_principal_num.replace("\D", ""), user_id))


            # Se for ATENDENTE e houver dados de 'detalhes' no payload
            if request.current_user['tipo_usuario'] == 'ATENDENTE' and 'detalhes' in data:
                ...
                # Lista de campos permitidos para atualização em atendente_detalhes
                ...
                for field in allowed_detalhes_fields:
                    if field in detalhes_data: # Permite que campos sejam enviados como nulos/vazios para limpar
                ...
                if campos_detalhes_para_atualizar:
                    ...
                    # Verificar se já existe registro em atendente_detalhes
                    ...
                    else: # Se não existir (caso raro, deveria ter sido criado no registro), insere
            ...
            # Retornar o nome atualizado para o localStorage
...
def alterar_senha_usuario(user_id):
    ...
    if not validate_senha(nova_senha): # Supondo que validate_senha existe
    ...
    try:
        with connection.cursor() as cursor:
            ...
            if not check_password(senha_atual, user['senha']): # Supondo que check_password existe
                return jsonify({'message': 'Senha atual incorreta.'}), 403
            
            hashed_nova_senha = hash_password(nova_senha) # Supondo que hash_password existe
...
# Rota para admin listar todos os usuários (simplificada)
@usuarios_bp.route('', methods=['GET'])
@admin_required
def get_todos_usuarios():
    # Adicionar filtros por tipo, situacao, busca (nome, email, cpf)
...
# Rota para admin obter detalhes de um usuário específico (pode ser a mesma /me, mas com checagem de admin)
@usuarios_bp.route('/<int:user_id>', methods=['GET'])
@admin_required # Ou @token_required e verificar se é admin ou o próprio usuário
def get_usuario_especifico_admin(user_id):
    # Similar a /me, mas para qualquer user_id se for admin
    # (Reutilizar lógica de get_meu_perfil, passando user_id)
    connection = get_connection()
    try:
        with connection.cursor() as cursor:
            cursor.execute("""
            ...
            """, (user_id,))
            usuario_info = cursor.fetchone()
            # ... (buscar telefones, endereços, detalhes_atendente como em get_meu_perfil) ...
            if not usuario_info:
                return jsonify({'message': 'Usuário não encontrado.'}), 404
            return jsonify({'usuario': usuario_info}), 200 # Simplificado
    finally:
        if connection: connection.close()


# Rotas para Notificações (Exemplo em usuarios.py, poderia ser um novo blueprint)
...
def admin_alterar_status_usuario(user_id):
    ...
    if not novo_status or novo_status not in ['ATIVO', 'BLOQUEADO', 'INATIVO']: # Adicionar INATIVO se usar
        return jsonify({'message': 'Novo status inválido ou não fornecido.'}), 400
    
    if novo_status == 'BLOQUEADO' and not motivo: # Motivo obrigatório para bloquear
