...
document.addEventListener('DOMContentLoaded', function() {
    ...
    // Carregar atendentes ao iniciar
    carregarAtendentes();
    
    // Função para carregar atendentes da API
    function carregarAtendentes() {
        // Mostrar indicador de carregamento
        attendantsList.innerHTML = '<tr><td colspan="8" style="text-align: center;">Carregando...</td></tr>';
        
        // Construir URL com filtros
        let url = '/api/atendentes/pendentes';
        ...
        .catch(error => {
            ...
            attendantsList.innerHTML = '<tr><td colspan="8" style="text-align: center;">Erro ao carregar atendentes</td></tr>';
        });
    }
    ...
    function confirmAction() {
        ...
        if (actionType === 'approve') {
            // Chamar a API para aprovar o atendente
            fetch(`/api/atendentes/${selectedAttendant.id_usuario}/aprovar`, {
                ...fetchConfig,
                method: 'POST',
            ...
            .then(response => {
                if (!response.ok) {
                    throw new Error('Erro ao aprovar atendente');
                }
                return response.json();
            })
            .then(data => {
            ...
            .catch(error => {
                console.error('Erro:', error);
                alert('Erro ao aprovar atendente. Tente novamente.');
            });
        } else if (actionType === 'reject') {
            // Chamar a API para bloquear o atendente
            fetch(`/api/atendentes/${selectedAttendant.id_usuario}/bloquear`, {
                ...fetchConfig,
                method: 'POST',
                body: JSON.stringify({ motivo: reason })
            })
            .then(response => {
                if (!response.ok) {
                    throw new Error('Erro ao reprovar atendente');
                }
                return response.json();
            })
            .then(data => {
            ...
            .catch(error => {
                console.error('Erro:', error);
                alert('Erro ao reprovar atendente. Tente novamente.');
            });
        }
    }
    ...
    if (updateButton) {
        updateButton.addEventListener('click', carregarAtendentes);
    }
