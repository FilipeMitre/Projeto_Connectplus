...
document.addEventListener('DOMContentLoaded', function() {
    ...
    function fetchAttendants() {
        ...
        // Corrigindo o endpoint para usar /api/atendentes
        ...
        console.log('Buscando atendentes na URL:', apiUrl); // Log para debug

        fetch(apiUrl, fetchConfig)
            .then(response => {
                if (!response.ok) throw new Error(`Erro ao buscar atendentes: ${response.statusText}`);
                return response.json();
            })
            .then(data => {
                console.log('Dados recebidos da API:', data); // Log para debug
                
                // Verificar se data.atendentes existe, caso contrário, usar data diretamente se for um array
                if (data.atendentes) {
            ...
            .catch(error => {
                ...
                attendantsTbody.innerHTML = `<tr><td colspan="7" class="error-message">Erro ao carregar atendentes: ${error.message}</td></tr>`;
            });
    }
    ...
    function openActionModal(actionType, attendant) {
        ...
        // Carregar detalhes completos do atendente (da view `vw_atendentes_pendentes`, `aprovados` ou `bloqueados`)
        // As views que criei no SQL já devem trazer muitos detalhes.
        // Se precisar de mais, seria um fetch para `/api/admin/atendentes/${attendant.id_usuario}`
    ...
    confirmActionButton.addEventListener('click', function() {
        if (!selectedAttendantForAction) return;

        const action = this.dataset.actionType;
        const motivo = actionReasonTextarea.value.trim();

        if ((action === 'reject' || action === 'block') && !motivo) {
            alert('O motivo é obrigatório para esta ação.');
            actionReasonTextarea.focus();
            return;
        }

        let endpoint = '';
        let novoStatus = ''; // Para o log no backend

        if (action === 'approve') {
            endpoint = `/api/atendentes/${selectedAttendantForAction.id_usuario}/aprovar`;
            novoStatus = 'ATIVO';
        } else if (action === 'reject' || action === 'block') {
            endpoint = `/api/atendentes/${selectedAttendantForAction.id_usuario}/bloquear`; // Corrigido para usar bloquear
            novoStatus = 'BLOQUEADO';
        } else if (action === 'unblock') {
            endpoint = `/api/atendentes/${selectedAttendantForAction.id_usuario}/aprovar`; // Desbloquear usa a mesma lógica de aprovar
            novoStatus = 'ATIVO';
        } else {
        ...
        const payload = {
            motivo: motivo,
            id_admin_responsavel: adminId // Envia o ID do admin logado
            // novo_status: novoStatus // O backend já sabe qual status aplicar
        };
        ...
        .then(response => {
            if (!response.ok) {
                return response.json().then(err => { throw new Error(err.message || `Erro ao executar ação ${action}`) });
            }
            return response.json();
        })
        .then(data => {
            alert(data.message || `Ação '${action}' realizada com sucesso para ${selectedAttendantForAction.nome_completo}!`);
            closeModal();
            fetchAttendants(); // Recarrega a lista
        })
        .catch(error => {
            console.error(`Erro ao ${action} atendente:`, error);
            alert(`Erro ao ${action} atendente: ${error.message}`);
        })
        .finally(() => {
