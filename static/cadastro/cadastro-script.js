...
document.addEventListener('DOMContentLoaded', function() {
    ...
    if (cadastroForm) {
        cadastroForm.addEventListener('submit', function(e) {
            ...
            // Preparar objeto com dados do usuário
            const userData = {
                nome: nome,
                cpf: cpf,
                email: email,
            ...
            // Se for atendente, adicionar campos específicos
            if (tipoUsuario === 'atendente') {
                ...
                userData.qualificacao = qualificacao;
                userData.area = areaAtuacao;
            }
            ...
            .then(response => {
                // Primeiro verificamos se a resposta é ok
                if (!response.ok) {
                    // Tentamos analisar como JSON, mas se falhar, usamos o texto
                    return response.json()
                        .catch(() => response.text().then(text => {
                            throw new Error('Erro ao processar a resposta: ' + text);
                        }))
                        .then(data => {
